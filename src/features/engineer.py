...
def extract_datetime_features(df: pd.DataFrame, ts_col: str, prefix: str) -> pd.DataFrame:
...
def extract_discount_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "lead_Скидка" in df.columns:
        ...
        df["discount_percent"] = df["lead_Скидка"] / df["lead_price"].replace(0, 1) * 100
...
def create_utm_aggregates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["has_utm"] = (~df["lead_utm_source"].isna() & (df["lead_utm_source"] != "Unknown")).astype(int)
...
def add_all_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = extract_datetime_features(df, "lead_created_at", "lead_created")
...
def add_geo_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "contact_Город" in df.columns:
        ...
        df["city_tier"] = df["contact_Город"].apply(
            lambda x: "top_city" if x in top_cities[:10] 
            else "mid_city" if x in top_cities 
            else "other"
        )
        df["city_is_moscow"] = df["contact_Город"].str.contains("Москва|Moscow", case=False, na=False).astype(int)
        df["city_is_spb"] = df["contact_Город"].str.contains("Петербург|Spb|Saint", case=False, na=False).astype(int)
