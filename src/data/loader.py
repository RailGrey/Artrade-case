...
def filter_post_payment(df: pd.DataFrame) -> pd.DataFrame:
    """Filter to only include post-payment orders (95% of data)."""
    if PAYMENT_FILTER_COL in df.columns:
        original_size = len(df)
        df = df[df[PAYMENT_FILTER_COL] == PAYMENT_FILTER_VALUE].copy()
        filtered_size = len(df)
        print(
            f"Filtered to post-payment: {filtered_size}/{original_size} "
            f"({filtered_size / original_size:.1%})"
        )
    return df
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Filter to post-payment only (cash on delivery)
    df = filter_post_payment(df)

    # Remove rows where outcome is unknown
    if "outcome_unknown" in df.columns:
        df = df[df["outcome_unknown"] == False]
        print(f"Removed unknown outcomes: {len(df)} remaining")

    # Rename target to consistent name
    if "buyout_flag" in df.columns:
        df[TARGET_COL] = df["buyout_flag"]
